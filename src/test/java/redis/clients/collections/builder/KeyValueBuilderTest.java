...
public class KeyValueBuilderTest {
	...
	public void shouldCreateKeyValueStruture() {
		keyValueRedisStructure<Person> personStructure =  RedisStrutureBuilder.ofKeyValue(RedisConection.JEDIS, Person.class).withNameSpace("person").withttl(120).build();
	...
	public void shouldReturnErrorWhenNameSpaceWasNotInformed() {
		keyValueRedisStructure<Person> personStructure =  RedisStrutureBuilder.ofKeyValue(RedisConection.JEDIS, Person.class).withttl(120).build();
		Assert.assertNotNull(personStructure);
	}
}
